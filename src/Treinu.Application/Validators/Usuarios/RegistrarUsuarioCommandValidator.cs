...
public class RegistrarUsuarioCommandValidator : AbstractValidator<RegistrarUsuarioCommand>
{
    ...
    {
        RuleFor(x => x.NomeCompleto).NotEmpty().MinimumLength(3).WithMessage("Nome completo deve ter pelo menos 3 caracteres.");
        ...
        RuleFor(x => x.AceiteTermoAdesao).Equal(true).WithMessage("Você deve aceitar os termos de adesão para se registrar.");
    }
}
