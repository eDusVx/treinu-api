...
public class CustomMediator : IMediator
{
    ...
    public async Task<TResponse> Send<TResponse>(IRequest<TResponse> request, CancellationToken cancellationToken = default)
    {
        ...
        if (handler == null)
            throw new InvalidOperationException($"Nenhum handler registrado para {requestType.Name}. O injetor de dependências não varreu o assembly ou a interface não bate.");
        ...
        foreach (var pipeline in Enumerable.Reverse(pipelines))
        {
            ...
            next = () => (Task<TResponse>)pipelineMethod.Invoke(pipeline, new object[] { request, nextClosure, cancellationToken })!;
        }
    ...
    public async Task Publish<TNotification>(TNotification notification, CancellationToken cancellationToken = default) where TNotification : INotification
    {
        ...
        var tasks = handlers.Select(handler => (Task)method.Invoke(handler, new object[] { notification, cancellationToken })!);
